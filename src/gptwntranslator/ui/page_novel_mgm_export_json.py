...
class PageNovelMgmExportJson(PageBase):
    ...
    def render(self, screen, **kwargs) -> tuple[PageBase, dict]:
        ...
        output = os.path.join(config.vars["output_path"], f"{novel_origin}-{novel_code}-{target_language}.json")

        # Print title
        last_y = print_title(screen, resources["title"], 0)
        
        last_y += 2
        last_y = print_messages(screen, [f"Exporting JSON: {output}"], 2, last_y)
        ...
        while True:
            ...
            try:
                message = "(1/2) Exporting novel to json... "
                screen.print_at(message, 2, last_y)
                screen.refresh()
                json_string = make_printable(json.dumps(novel, ensure_ascii=False, cls=JsonEncoder))
                ...
                last_y += 1
                messages = [
                    f"Error: Error exporting novel to json.",
                    f"Error: {e}"]
                target = PageMessage
                params = {"messages": messages, "return_page": self.args["return_page"], "return_kwargs": self.args["return_kwargs"]}
            break
